...
static t_class *granufm_class = NULL;
...
t_float CLAMP(t_float val, t_float min, t_float max)
{
    val = ( val >= min ) ? val : min;
    val = ( val <= max ) ? val : max;
    return val;
...
t_int *grans_perform(t_int *w_ptr)
{
    /*
     single evelope version:
     0 trigger
...
/*
	for (i = 0; i < numins; ++i)
		in[i] = (t_float *)(w[i+3]);
  */
    t_sample *outlets[numouts];
	for (i = 0; i < numouts; i++)
		outlets[i] = (t_sample *)(w[i + numins + 3]);
    ...
    double *sinetab = granu_sinetab; //not sure if the const is helping me a lot?
    const double *cos_wind = granu_wind_costab;
    const double *decaytab = granu_expdecaytab;
    const double *dampedsinetab = granu_dampedsinetab;
    const double *sinc_wind = granu_sincwindow;
    ...
    for( i = 0; i < maxoscillators; i++, o++)
    {
        ...
        else if( w_type == 2 ) //dampedsine window
        {
            while(j < sampleframes && wpc < STABSZ)
            {
                ...
                amp = granu_exptab[ yshapetab ][
                                          (uint32_t)(dampedsinetab[(uint32_t)(granu_exptab[ xshapetab ][ (uint32_t)wpc ] * wtabscale)] * wtabscale)
                                          ];
                outlets[outletnum][j] += amp * FM(sinetab, STABSZ, pc, harm[j], modindex[j] );
                pc = fmod((pc + pi), STABSZ);
                wpc += wpi;
        ...
        else if( w_type == 3 ) //sinc window
        {
            while(j < sampleframes && wpc < STABSZ)
            {
                ...
                amp = granu_exptab[ yshapetab ][
                                          (uint32_t)(sinc_wind[(uint32_t)(granu_exptab[ xshapetab ][ (uint32_t)wpc ] * wtabscale)] * wtabscale)
                                          ];
                outlets[outletnum][j] += amp * FM(sinetab, STABSZ, pc, harm[j], modindex[j] );
                pc = fmod((pc + pi), STABSZ);
                wpc += wpi;
